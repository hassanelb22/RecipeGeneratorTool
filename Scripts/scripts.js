...
function sharePost() {
  ...
  const facebookShareURL = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
    recipeUrl
  )}&t=${encodeURIComponent(recipeTitle)}`;
...
function showAlert(message) {
  ...
  alertBox.className =
    "fixed bottom-5 left-5 bg-green-500 text-white py-2 px-4 rounded-lg shadow-lg opacity-0 transform translate-y-4 transition-all duration-500";
...
function downloadCSV() {
  ...
  savedRecipes.forEach(({ name, content, language, prompts }) => {
    ...
    csvContent += `"${name}","${content.replace(/"/g, '""')}","${language}","${ahmadBatPrompt.replace(/"/g, '""')}","${anouarSaidiPrompt.replace(/"/g, '""')}","${ahmadJdayPromptV1.replace(/"/g, '""')}","${ahmadJdayPromptV2.replace(/"/g, '""')}"\n`;
  });
...
// Download CSV functionality (for full recipe data)
function downloadCSV() {
  let savedRecipes = JSON.parse(localStorage.getItem("recipes")) || [];
  if (savedRecipes.length === 0) {
    alert("No recipes to download.");
...
function downloadCanvaCSV() {
  ...
  // Define CSV headers for Canva template
  let csvContent = "Recipe Name,Ingredients,Images\n";
  ...
  let link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = "canva_template.csv";
  link.click();
}

// Helper function to extract ingredients from recipe content
function extractIngredients(recipeContent) {
  // Example: Look for a line starting with "Ingredients:" and extract the list
  const ingredientsMatch = recipeContent.match(/Ingredients:\s*([\s\S]*?)\n\n/);
  if (ingredientsMatch) {
    // Split the ingredients by commas or newlines
